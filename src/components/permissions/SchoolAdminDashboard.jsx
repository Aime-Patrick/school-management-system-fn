...
const SchoolAdminDashboard = () => {
  ...
  // School Admin specific data - Get schoolId from auth context
  const schoolId = authData?.schoolId;

  // Get users for THIS school only
  const { 
    data: usersWithPermissions, 
    isLoading: usersLoading,
    error: usersError 
  } = useUsersWithPermissions(schoolId, false); // false = not system admin

  // School Admins can access permission sets
  const { 
    data: permissionSets, 
  ...
  } = useUserPermissions(selectedUser, schoolId, false); // false = not system admin
  ...
  const resources = [
    ...
    'EVENTS', 'ACADEMIC_YEARS', 'TERMS', 'USERS', 'FINANCIAL', 'REPORTS'
  ];

  const actions = ['CREATE', 'READ', 'UPDATE', 'DELETE', 'VIEW'];
  // School Admins cannot manage system-admin or other school-admin roles
  const availableRoles = ['teacher', 'student', 'librarian', 'accountant', 'parent'];
  ...
  // Table columns for users - NO school column needed for school admins
  const userColumns = [
    {
      title: 'User',
  ...
  const stats = {
    ...
    teachers: usersWithPermissions?.filter(u => u.user?.role === 'teacher').length || 0,
    students: usersWithPermissions?.filter(u => u.user?.role === 'student').length || 0,
  };
    ...
    return (
      <div className="p-6">
        <Alert
          ...
          description="Failed to load user data for your school. Please try again."
          type="error"
          showIcon
        />
      </div>
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-6">
          <div className="flex justify-between items-center mb-4">
            <div>
              ...
              <Text type="secondary">
                Manage permissions for users in your school
              </Text>
            </div>
            <Space>
              <Button 
          ...
          {/* School Admin Notice */}
          <Alert
            message="School Administrator Access"
            description="You can only manage permissions for users within your school. You cannot grant system-admin permissions or access other schools."
            type="warning"
            showIcon
            className="mb-4"
          />
        ...
        <Tabs
          ...
          items={[
            {
              key: 'users',
              label: 'School Users & Permissions',
              children: (
            ...
            {
              key: 'audit',
              label: 'School Audit Trail',
              children: (
                <Card className="bg-white rounded-lg shadow">
                  <div className="p-6">
                    <Alert
                      message="School Audit Trail"
                      description="Track all permission changes within your school"
                      type="info"
                      showIcon
                      className="mb-4"
                    />
                    <Table
                      ...
                      dataSource={[]} // TODO: Add school audit trail
                      rowKey="id"
                      pagination={{
                        pageSize: 10,
                        showSizeChanger: true,
      ...
      <Modal
        title={`User Permissions - ${selectedUserPermissions?.user?.username || 'Loading...'}`}
        open={isModalVisible}
        onCancel={() => {
                ...
                >
                  ...
                  {setsError && (
                    <Alert
                      ...
                      description="Unable to load permission sets. Please try again."
                      type="error"
                      showIcon
                      className="mb-4"
                    />
